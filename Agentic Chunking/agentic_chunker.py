...
class AgenticChunker:
    def __init__(self, openai_api_key=None):
        ...
        self.llm = ChatOpenAI(model='llama3', openai_api_key="NA", temperature=0)
    ...
    def add_proposition(self, proposition):
        ...
        # If it's the first chunk, just make a new chunk and don't check for others
        if len(self.chunks) == 0:
            if self.print_logging:
                print ("No existing chunks, creating a new one...")
        ...
        PROMPT = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    You will be given a proposition related to Surface Mount Technology in electronics manufacturing which will go into a new chunk. This new chunk needs a summary.

                    Your summaries should anticipate verbatim. For example, if you get a proposition about soldering, use the same word 'solder' while summarizing.
                    Use context-relevant appropriate engineering(electronics) terms while generalizing on the details of electronics production.

                    Example:
                    Input: Proposition: The amount of solder is measured by the volume of the solder joints. As the area is the same, it is enough to 
                                        measure the height of the solder to determine the amount of soldering material. 
                    Output: This chunk contains information about determining the amount of solder required for specific volumes.

                    ONLY respond with the new chunk summary, DO NOT RETURN ANYTHING ELSE. 
                    """,
                ),
        ...
        PROMPT = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    Your titles should anticipate generalization based on the overall context. 
                    
                    Example:
                    Input: Summary: This chunk is about adjusting soldering temperatures for strong electrical conductivity.
                    Output: Soldering Temperature Criteria
    ...
    def _create_new_chunk(self, proposition):
        new_chunk_id = str(uuid.uuid4())[:self.id_truncate_limit] # to limit the length of IDs
        ...
        if self.print_logging:
            print (f"Created new chunk (Chunk ID: {new_chunk_id}) | Title: {new_chunk_title}")
